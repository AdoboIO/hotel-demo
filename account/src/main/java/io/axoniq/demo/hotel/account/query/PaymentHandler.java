...
package io.axoniq.demo.hotel.account.query;
...
import io.axoniq.demo.hotel.account.command.web.api.PaymentResponseData;
import io.axoniq.demo.hotel.account.command.web.api.PaymentSucceededEvent;
import io.axoniq.demo.hotel.account.query.api.FindPayment;
import io.axoniq.demo.hotel.account.query.api.FindPayments;
import io.axoniq.demo.hotel.account.query.api.FindPaymentsForAccount;
import io.axoniq.demo.hotel.account.query.api.PaymentEntity;
...
class PaymentHandler {
    ...
    private PaymentResponseData convert(PaymentEntity entity) {
        return new PaymentResponseData(entity.getPaymentId(),
                                       entity.getAccountId(),
    ...
    void on(PaymentRequestedEvent event) {
        PaymentEntity entity = new PaymentEntity(event.getPaymentId().toString(),
                                                 event.getAccountId(),
    ...
    List<PaymentResponseData> handle(FindPaymentsForAccount query) {
        return paymentEntityRepository.findByAccountId(query.getAccountId()).stream().map(this::convert).collect(
                Collectors.toList());
    }
}
