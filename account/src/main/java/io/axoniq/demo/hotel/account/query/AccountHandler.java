...
import io.axoniq.demo.hotel.account.command.web.api.AccountResponseData;
import io.axoniq.demo.hotel.account.query.api.AccountEntity;
import io.axoniq.demo.hotel.account.query.api.FindAccount;
import io.axoniq.demo.hotel.account.query.api.FindAccounts;
...
class AccountHandler {
    ...
    void on(AccountRegisteredEvent event) {
        AccountEntity entity = new AccountEntity(event.getAccountId().toString(),
                                                 event.getUserName(),
                                                 event.getPassword());
    ...
    AccountResponseData handle(FindAccount query) {
        return convert(accountEntityRepository.findById(query.getAccountId().toString())
                                              .orElseThrow(() -> new InvalidParameterException(query.getAccountId()
                                                                                                    .toString())));
    }
